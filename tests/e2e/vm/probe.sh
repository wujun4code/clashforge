...
if [ "$PHASE" = "running" ]; then
    ...
    ROUTER_TARGETS="\
https://www.taobao.com:淘宝:国内:用于验证国内主要电商平台的直连可达性。\
 https://music.163.com:网易云音乐:国内:用于验证国内常见内容站点延迟。\
 https://github.com:GitHub:国外:用于验证国际开发站点的代理访问效果。\
 https://www.google.com:Google:国外:用于验证 Google 搜索是否可通过代理访问。\
 https://chat.openai.com:OpenAI:AI:用于验证 ChatGPT / OpenAI 是否可通过代理访问。\
 https://api.anthropic.com:Claude:AI:用于验证 Claude AI 是否可通过代理访问。\
 https://gemini.google.com:Gemini:AI:用于验证 Google Gemini 是否可通过代理访问。"

    RD_OK_COUNT=0; RD_TOTAL=0

    for entry in $ROUTER_TARGETS; do
