...
case_03_stop_reset() {
  ...
  run_ctl "stop exits takeover" stop --yes || return 1
  assert_vm_ok "mihomo stopped after stop" "! pgrep -f mihomo-clashforge >/dev/null 2>&1" || return 1

  run_ctl "reset --start succeeds" reset --start --yes || return 1
